...
Component({
    ...
    data: {
      type: Object, observer(params, oldParams) {
        if (params.show === true) {
          ...
          if (!params.animation) params.animation = (config.animation || "none");
          ...
          if (typeof params.maskClose !== "boolean") params.maskClose = (config.maskClose || false);// 允许点击蒙版关闭
          ...
          /* style */
          params.confirmStyle = "", params.cancelStyle = "";
          if (params.confirmColor) params.confirmStyle += `color: ${params.confirmColor};`;
          if (params.cancelColor) params.cancelStyle += `color: ${params.cancelColor};`;
  ...
  methods: {
    show() {
      this.setData({
        animationClass: this.data.p.animation + "-show",
    ...
    hide(success) {
      this.setData({
        animationClass: this.data.p.animation + "-hide",
        toggleClass: "gmodal-hide",
        "p.show": false
      });
      setTimeout(() => {
      ...
      }, this.data.p.animation === "none" ? 0 : 350);
    },

    isTrigger: false,
    /** 绑定确定按钮[tap]事件 */
    bindConfirm(event) {
      if (this.isTrigger) { console.log(new Date().getTime()); return };
      let detail = {};
